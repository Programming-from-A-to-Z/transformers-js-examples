      ...
      /***/ function (module, exports, __webpack_require__) {
        ...
        function makeLeafArray(rpForest) {
          ...
          if (rpForest.length > 0) {
            ...
            try {
              for (
                var rpForest_1 = __values(rpForest), rpForest_1_1 = rpForest_1.next();
                !rpForest_1_1.done;
      ...
      /***/ function (module, exports, __webpack_require__) {
        ...
        var UMAP = (function () {
          ...
          UMAP.prototype.transform = function (toTransform) {
            ...
            var _b = this.smoothKNNDistance(distances, this.nNeighbors, adjustedLocalConnectivity),
              sigmas = _b.sigmas,
              rhos = _b.rhos;
            var _c = this.computeMembershipStrengths(indices, distances, sigmas, rhos),
              rows = _c.rows,
              cols = _c.cols,
              vals = _c.vals;
          ...
          UMAP.prototype.fuzzySimplicialSet = function (X, nNeighbors, setOpMixRatio) {
            ...
            var _d = this.smoothKNNDistance(knnDistances, nNeighbors, localConnectivity),
              sigmas = _d.sigmas,
              rhos = _d.rhos;
            var _e = this.computeMembershipStrengths(knnIndices, knnDistances, sigmas, rhos),
              rows = _e.rows,
              cols = _e.cols,
              vals = _e.vals;
            ...
            for (var i = 0; i < epochsPerSample.length; i++) {
              ...
              for (var p = 0; p < nNegSamples; p++) {
                ...
                if (distSquared_1 > 0.0) {
                  ...
                  gradCoeff_1 /= (0.001 + distSquared_1) * (a * Math.pow(distSquared_1, b) + 1);
                } else if (j === k_1) {
                  continue;
                }
                for (var d = 0; d < dim; d++) {
      ...
      /***/ function (module, __webpack_exports__, __webpack_require__) {
        ...
        function rescale(input) {
          ...
          var _options$min = options.min,
            minValue =
              _options$min === void 0 ? (options.autoMinMax ? currentMin : 0) : _options$min,
            _options$max = options.max,
            maxValue =
              _options$max === void 0 ? (options.autoMinMax ? currentMax : 1) : _options$max;
...
${indent}columns: ${matrix.columns}
        ...
        function installMathOperations(AbstractMatrix, Matrix) {
          ...
          AbstractMatrix.prototype.signPropagatingRightShift = function signPropagatingRightShift(
            value
          ) {
          ...
          AbstractMatrix.prototype.signPropagatingRightShiftS = function signPropagatingRightShiftS(
            value
          ) {
            for (let i = 0; i < this.rows; i++) {
              for (let j = 0; j < this.columns; j++) {
          ...
          AbstractMatrix.prototype.signPropagatingRightShiftM = function signPropagatingRightShiftM(
            matrix
          ) {
            matrix = Matrix.checkMatrix(matrix);
            if (this.rows !== matrix.rows || this.columns !== matrix.columns) {
          ...
          AbstractMatrix.signPropagatingRightShift = function signPropagatingRightShift(
            matrix,
            value
          ) {
            const newMatrix = new Matrix(matrix);
        ...
        class matrix_AbstractMatrix {
          ...
          subMatrix(startRow, endRow, startColumn, endColumn) {
            ...
            let newMatrix = new matrix_Matrix(endRow - startRow + 1, endColumn - startColumn + 1);
            ...
            while (p > 0) {
              ...
              if (k === p - 2) {
                ...
                for (ks = p - 1; ks >= k; ks--) {
                  ...
                  let t =
                    (ks !== p ? Math.abs(e[ks]) : 0) + (ks !== k + 1 ? Math.abs(e[ks - 1]) : 0);
